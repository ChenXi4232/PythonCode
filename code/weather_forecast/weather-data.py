...
starturl = "http://www.tianqihoubao.com/aqi/hefei.html"
html = urlopen(starturl)
bsobj = BeautifulSoup(html, "html.parser")  # 获取BeautifulSoup对象

# 找到所有存放月度数据的网页链接，并以列表的形式按月份先后顺序保存这些链接
Sites = []
for link in bsobj.findAll(href=re.compile("^(/aqi/hefei-)")):
