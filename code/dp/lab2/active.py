...
class LinearActive:
    ...
    def _data_grad_naive(self, residue, batch_idxes):
        data_grad = -1 * (residue[:, np.newaxis] *
    ...
    def train(self, trainset):
        ...
        for epoch in tbar:
            ...
            for batch in range(n_batches):
                ...
                start = batch * bs
                end = len(all_idxes) if batch == n_batches - \
                    1 else (batch + 1) * bs
                ...
                # -----------------------------------------------------------------
                active_wx = np.dot(
                    self.x_train[batch_idxes], self.params)  # compute theta^A * x^A
                passive_wx = self.messenger.recv()
                full_wx = active_wx + passive_wx  # compute theta^A * x^A + theta^B * x^B
                ...
                # -----------------------------------------------------------------
                enc_residue = self.cryptosystem.encrypt_vector(
                    residue)  # encrypt residue from A
                enc_residue = np.array(enc_residue)
                self.messenger.send(enc_residue)
                enc_passive_grad = self.messenger.recv()
                passive_grad = self.cryptosystem.decrypt_vector(
                    enc_passive_grad)  # decrypt gradient from B
        ...
        with open("accs.txt", "w") as f:
            f.write("\n".join([str(acc) for acc in accs]))

        # Plot losses and accs
        plt.plot(losses, label="loss")
        plt.plot(accs, label="acc")
        plt.xlabel("Epoch * n_batches")
    ...
    def _acc(self, y_true, y_hat):
        # Q3. Compute accuracy
        # -----------------------------------------------------------------
        acc = np.mean(np.round(y_hat) == y_true)  # compute accuracy
        # -----------------------------------------------------------------
