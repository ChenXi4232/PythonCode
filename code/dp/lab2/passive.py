...
class LinearPassive:
    ...
    def _data_grad_naive(self, enc_residue, batch_idxes):
        data = self.x_encode
        enc_train_grad = -1 * \
            (enc_residue[:, np.newaxis] * data[batch_idxes]).mean(axis=0)
    ...
    def _mask_grad(self, enc_grad):
        mask = np.random.normal(0, 1.0, enc_grad.shape)  # random masks
        enc_mask_grad = enc_grad + \
            self.cryptosystem.encrypt_vector(
                mask)  # encrypted random masks, add to the gradient
        return enc_mask_grad, mask

    def _unmask_grad(self, mask_grad, mask):
        true_grad = mask_grad - mask  # remove the random masks
        return true_grad
    ...
    def train(self, trainset):
        ...
        for epoch in tbar:
            ...
            for batch in range(n_batches):
                ...
                # -----------------------------------------------------------------
                passive_wx = np.dot(
                    self.x_train[batch_idxes], self.params)  # compute theta^B * x^B
